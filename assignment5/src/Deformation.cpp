...
void Deformation::get_deformed_mesh(const Eigen::MatrixXd &handle_vertex_positions, Eigen::MatrixXd &V_res) {
    ...
    MatrixXd displacement = V_original - V_smooth;
    //displacements decomposition to get component
    MatrixXd component(V_smooth.rows(), 3), N;
    Vector3d Ni, vi, vj, x_axis, y_axis;
    per_vertex_normals(V_smooth, F, N);
    vector<vector<int>> neighbors;
    adjacency_list(F, neighbors);

    //correspondent longest edge
    vector<int> v_index(V_smooth.rows());
    ...
    //add details back
    MatrixXd N_prime, displacement_prime(V_deformed_smooth.rows(), 3);
    Vector3d N_i_prime, vi_prime, vj_prime, x_axis_prime, y_axis_prime;
    per_vertex_normals(V_deformed_smooth, F, N_prime);
    for (int i = 0; i < V_deformed_smooth.rows(); ++i) {
        N_i_prime = N_prime.row(i);
        vi_prime = V_deformed_smooth.row(i);
        vj_prime = V_deformed_smooth.row(v_index[i]);
    ...
    //S' = B' + D
    V_res = V_deformed_smooth + displacement_prime;
}
